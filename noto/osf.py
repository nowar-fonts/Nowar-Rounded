...
if __name__ == '__main__':
	...
	with open("rounded/NotoRounded-{}.otd".format(width + weight), 'rb') as baseFile:
		baseFont = json.loads(baseFile.read().decode('UTF-8', errors='replace'))

	for entry in baseFont['name']:
		entry['nameString'] = entry['nameString'].replace('Noto Rounded', 'Noto Rounded Oldstyle').replace('Noto-Rounded', 'Noto-Rounded-Oldstyle')
	...
	with open("osf/NotoRoundedOSF-{}.otd".format(width + weight.replace(' ', '')), 'w') as outFile:
